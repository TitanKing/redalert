        ...
        if self.output_listen():
            if state.active_zone:
                ...
                if self.input_listen_reset():
                    ...
                    print("No reset triggered yet...")
                    # print(state.active_zone)
        elif self.zones():
            if state.active_zone:
                print("Outputs activated")
                self.output()
    ...
    def output_timeout(self):
        if self.io.active_zone and self.io.trigger_time:
            ...
            for output_pin, active_output_time in self.io.trigger_time.items():
                if str(az["output_timeout"]) != "null" and float(az["output_timeout"]) > 0 and float(self.io.trigger_time[output_pin]) > 0:
                    print("Output Timeout Pin: " + str(output_pin) + " at time " + str(self.io.trigger_time[output_pin]))
